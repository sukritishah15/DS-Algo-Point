// Top view of a Binary Tree Solution from Scratch
...
void print_top_view(TreeNode* root, map<int,int>& m1)
{
    ...
    while(!q1.empty())
    {
        //We'll be saving the value of node as value of it's corresponding key, i.e. vertical level in map only for the first time(so as to get the top view)
        if(m1.find(q1.front().second)==m1.end())
...
int main()
{
    ...
    map<int, int> m1;
    print_top_view(root,m1);

    //Printing the top view of the tree
    cout<<"Printing the top view of the tree: ";
...
Printing the top view of the tree: 4 7 1 10 
*/
